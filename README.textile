...
Paneslide is a jQuery plugin that slides a pane in from the left or right. Heavily influenced by and based on jquery-paneslide by Scott Robbin.
...
By default, when paneslide opens, you can click on the document to close it. If modal is set to 'true', then you must explicitly close paneslide using $.paneslide.close(); (default=false)
